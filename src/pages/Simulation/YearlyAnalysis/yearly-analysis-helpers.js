...
export const buildAnnualContribution = (enriched) =>
  enriched
    ...
    .map((s) => ({
      ...
      temperature: s.temperature,
      occurrences: s.occurrences,
      dailyDrv: fin(s.kpis?.drivetrainEnergyKwh) ?? 0,
      dailyAux: fin(s.kpis?.auxiliaryEnergyKwh) ?? 0,
      dailyTotal: fin(s.kpis?.totalEnergyKwh) ?? 0,
      dailyQ05: fin(s.kpis?.quantiles?.q05),
      dailyQ50: fin(s.kpis?.quantiles?.q50),
      dailyQ95: fin(s.kpis?.quantiles?.q95),
